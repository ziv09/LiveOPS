...
function getRoleGroupFromDisplayName(displayName) {
...
function buildSlots() {
    ...
    for (let i = 1; i <= 16; i += 1)
        slots.push({ slotId: `collector_${String(i).padStart(2, '0')}`, role: 'collector' });
    for (let i = 1; i <= 4; i += 1)
        slots.push({ slotId: `monitor_${String(i).padStart(2, '0')}`, role: 'monitor' });
    for (let i = 1; i <= 5; i += 1)
        slots.push({ slotId: `crew_${String(i).padStart(2, '0')}`, role: 'crew' });
    return slots;
}
const SLOTS = buildSlots();
function compactState(input, now) {
    const state = input?.allocations ? input : { updatedAt: 0, allocations: {} };
    const nextAlloc = {};
    for (const [slotId, a] of Object.entries(state.allocations ?? {})) {
        if (!a?.uid || !a?.lastSeen)
            continue;
        if (now - Number(a.lastSeen) > TTL_MS)
            continue;
...
function pickSlotForUid(state, uid) {
    for (const a of Object.values(state.allocations)) {
        if (a.uid === uid)
            return a;
    }
...
function pickFreeSlot(state, role) {
    const tryPick = (r) => {
        for (const s of SLOTS) {
            if (s.role !== r)
                continue;
            if (!state.allocations[s.slotId])
...
export const issueJaasToken = onCall({ secrets: [JAAS_PRIVATE_KEY, JAAS_KID, JAAS_TENANT_ID] }, async (req) => {
    if (!req.auth?.uid)
        throw new HttpsError('unauthenticated', '需要先登入（Firebase Auth）。');
    const ops = normalizeOps(req.data?.ops);
    const displayName = String(req.data?.displayName ?? '').trim();
    const requestedRole = String(req.data?.role ?? '').trim().toLowerCase();
    if (!ops)
        throw new HttpsError('invalid-argument', '缺少 ops。');
    if (!displayName)
        throw new HttpsError('invalid-argument', '缺少 displayName。');
    ...
    if (!privateKeyPem || !privateKeyPem.includes('BEGIN')) {
        throw new HttpsError('failed-precondition', 'Cloud Functions 尚未設定 JAAS_PRIVATE_KEY（必須是 PEM 私鑰內容）。');
    }
    if (!kid)
        throw new HttpsError('failed-precondition', 'Cloud Functions 尚未設定 JAAS_KID。');
    if (!tenantId)
        throw new HttpsError('failed-precondition', 'Cloud Functions 尚未設定 JAAS_TENANT_ID。');
    const roomRef = admin.database().ref(`liveops/v2/jaas/rooms/${ops}/state`);
    const now = Date.now();
    const result = await roomRef.transaction((current) => {
        const cleaned = compactState(current, now);
    ...
    const finalState = compactState(result.snapshot.val(), now);
...
export const releaseJaasSlot = onCall(async (req) => {
    if (!req.auth?.uid)
        throw new HttpsError('unauthenticated', '需要先登入（Firebase Auth）。');
    const ops = normalizeOps(req.data?.ops);
    const slotId = String(req.data?.slotId ?? '').trim();
    if (!ops)
        throw new HttpsError('invalid-argument', '缺少 ops。');
    if (!slotId)
        throw new HttpsError('invalid-argument', '缺少 slotId。');
